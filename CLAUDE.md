...
> **Cost summary on completion (migration 036):** When `review_completion()` approves a job, it prorates `payments.amount` by `actual_hours / booked_hours` and stores `final_amount` + `refund_amount` + `actual_hours_worked` atomically in the same transaction. Overtime is **never** auto-charged (`actual_hours` is clamped to `booked_hours`); customers can optionally tip via `POST /assignments/{id}/tip`. `refund_amount` is computed here but the actual bank transfer is tracked via migration 042 workflow below.
> **Refund tracking workflow (migration 042):** `booking.payments` has `refund_status ('pending'|'processed'|'skipped')` + `refund_processed_at` + `refund_reference` (bank slip) + `refund_processed_by` (admin user_id). `prorate_payment_in_tx` auto-sets `refund_status='pending'` when `refund_amount > 0`. Admin uses `PUT /booking/admin/refunds/{id}/process` with `{action: 'process'|'skip', reference?, note?}` to mark refund complete (or skipped). Transitioning out of `pending` is one-way — re-opening needs a separate "reopen" flow (not scoped). Admin-only endpoints: `GET /admin/wallet/summary`, `GET /admin/payments`, `GET /admin/payments/{id}`, `GET /admin/refunds?status=`, `PUT /admin/refunds/{id}/process`. `require_admin()` helper rejects non-admin with 403.
...
- **IndexedStack tab refresh pattern (BUG-022 Task 35):** Dashboard screens that use `IndexedStack` (e.g. `GuardDashboardScreen`, `HirerDashboardScreen`) keep ALL tab children mounted — each tab's `initState` fires exactly once at dashboard mount. When server-side data changes while user is on another tab (e.g. new `pending_acceptance` arrives via FCM), the receiving tab's cached state stays stale on switch-in. **Solution:** route ALL tab-switch entry points (home-tile buttons + `BottomNavigationBar.onTap`) through a single `_switchTab(int index)` method that refreshes the relevant provider slice on switch: `index == 1` (jobs) → `fetchJobs()`, `index == 0` (home) → `fetchDashboard()`. Fire-and-forget — provider's `notifyListeners()` drives the rebuild.
...
- `POST /profile/avatar` → `handlers::update_avatar` (any JWT, multipart, field name `avatar`) — replaces own avatar. Stores S3 key in `auth.users.avatar_url`; `get_profile()` presigns on read. Validates size + magic bytes (JPEG/PNG/WEBP, ≤10MB). Returns full `UserResponse` with rewritten signed URL.
- `GET /profile/guard/info` → `handlers::get_guard_info` (guard JWT, self) — returns `GuardProfileResponse` (full incl. bank, address, emergency contact, signed doc URLs). Account number masked. Mobile uses this to populate the profile-edit form (separate from `/me` which excludes sensitive fields).
- `PUT /profile/guard/info` → `handlers::update_guard_info` (guard JWT, self) — guard self-edit of `auth.guard_profiles` non-doc fields. Body shape = `AdminUpdateGuardProfileRequest` (gender, dob, experience, workplace, bank info, address, emergency contact, expiry dates). COALESCE merges with existing values; only changed fields need to be sent. Reuses `service::admin_update_guard_profile()` with `user.user_id` from JWT (not path).
- `PUT /profile/guard/document/{doc_type}` → `handlers::update_guard_document` (guard JWT, self, multipart, field name `file`) — guard re-uploads single doc image (id_card, security_license, training_cert, criminal_check, driver_license). Reuses `upload_document()` so watermark "FOR SECURITY USE ONLY" is applied. **Does NOT revert approval status** — approved guard can swap doc images freely without admin re-review (per product policy 2026-04-30).
...
- `GET /me` → `handlers::get_me` (JWT required) — returns `UserResponse { id, full_name, phone, email, role, avatar_url, approval_status, customer_approval_status, customer_full_name }`. `full_name` = guard name from `auth.users`, `customer_full_name` = customer name from `customer_profiles` (separate, never merged). Used by `AuthProvider.fetchProfile()` for dashboard real data. **Avatar URL** is presigned on read via `finalize_avatar_url()` (cache stores raw S3 key; presigning runs on every cache hit + DB miss).
- `PUT /me` → `handlers::update_profile` (JWT required) — accepts `{full_name?, phone?, avatar_url?, email?}`. **Phone is intentionally NOT exposed in mobile UI** — it's the login identifier and changing it should require a re-OTP flow (not yet implemented). Email validates `@` + `.` + min length 5. COALESCE merges nulls.
...
- `PUT /assignments/{id}/start` → `handlers::start_job` (guard JWT) — guard starts accepted job. Sets `started_at = NOW()` only — **does NOT change status**. Assignment stays at `status='arrived'` through the entire active-work period until guard explicitly requests completion via separate endpoint (transitions to `pending_completion`). Frontend should detect "actively working" state via `status='arrived' && started_at != null` (not via `status='started'` which doesn't exist as a transition).
...
- `GET /customer/receipts` → `handlers::list_customer_receipts` (JWT, customer-only) — paginated past receipts (newest first). Scope: `assignments.status = 'completed' AND payments.status = 'completed'`. Query params: `limit` (default 20, max 100), `offset`. Returns `ReceiptsPage { data: [ReceiptItem...], total }`. Each `ReceiptItem` carries a derived `receipt_no` (`RCP-YYYY-<8-hex>`), guard name + avatar, service address, booked/actual hours, full price breakdown (original, final, refund, tip, net), payment method, and timestamps. Mobile `ReceiptListScreen` + `ReceiptDetailScreen` consume this; PDF generated client-side via `pdf` + `printing`.
- `GET /guard/active-job` → `handlers::get_active_job` (guard JWT) — returns active job for guard (accepted/in_progress assignment with request details, `started_at`, `booked_hours` for countdown, **`location_lat` and `location_lng` — customer's destination coordinates, NOT NULL since migration 002**, plus check-in coordinates `en_route_lat/lng`, `arrived_lat/lng`, `started_lat/lng` as `Option`). 3 backend handlers populate this shape: `start_job`, `get_active_job`, `get_customer_active_job` — all three must match the response struct (BUG-015 Issue B Task 29).
...
- `ApiClient`: auto-detects platform for default base URL — iOS → `http://localhost:80`, Android → `http://10.0.2.2:80` (override via `--dart-define=API_URL=...`). Skip auth for `/auth/otp/request`, `/auth/otp/verify`, `/auth/register/otp`, `/auth/profile/reissue`, `/auth/profile/guard`, `/auth/profile/customer`, `/auth/login/mobile`, `/auth/refresh/mobile`, `/auth/check-status`. **`/auth/profile/role` is intentionally NOT in publicPaths** — its backend handler does optional auth: if a Bearer token is present, skip the phone_verified_token requirement (lets approved guards add a customer profile without re-OTP). **401 interceptor calls `POST /auth/refresh/mobile`** (ไม่ใช่ `/auth/refresh`) — parse tokens จาก `response.data['data']` (ApiResponse wrapper). **Path matching uses `==` exact equality, not `.contains`** — sub-paths like `/auth/profile/guard/document/{type}` would otherwise inherit the public-skip rule of their parent `/auth/profile/guard` and lose their Bearer header.
- `ApiClient.rewriteMediaHost(url)`: static helper that swaps the origin of presigned MinIO URLs (or `auth.users.avatar_url`) so Android Emulator and physical devices can reach them. Backend signs with `S3_PUBLIC_URL` (defaults to `http://localhost/minio-files` in dev) — fine for web admin / iOS Simulator, but `localhost` resolves to the device itself on Android Emulator + physical phones. The helper detects "internal" hosts (`localhost`, `127.0.0.1`, `10.0.2.2`, `minio`) and replaces the origin with `ApiClient.baseUrl` (the platform-resolved host). Production R2 URLs pass through unchanged. Apply at every parse site (`fetchProfile`, `fetchGuardDocs`, `uploadAvatar`, `uploadGuardDocument`).
...
- `RoleSelectionScreen._onRoleTap()`: **authenticated users** → customer path checks `customerApprovalStatus`: `'approved'` → HirerDashboard, `'pending'` → RegistrationPendingScreen, `null` → CustomerRegistrationScreen; guard path gates on `auth.role` (authoritative DB value from `/auth/me`) — `auth.role == 'guard'` → GuardDashboard shortcut, otherwise **fall through to registration flow below** so non-guard users can upgrade (BUG-023 Tasks 36+37). **unauthenticated users** → calls `updateRole(phone, role)` → guard path gets profile_token → `GuardRegistrationScreen(phone, profileToken)`; customer path gets profile_token → `CustomerRegistrationScreen(phone, profileToken)` (uses `pushReplacement` ไม่ใช่ `push`)
...
- **Dashboard real data:** After login, dashboard screens use `AuthProvider` profile fields (`fullName`, `phone`, `avatarUrl`) from `GET /auth/me` instead of hardcoded mock data:
  ...
  - `CustomerTrackingScreen`: customer's pre-arrival tracking. **Polling progression (BUG-021 Task 34):** `_pollStatus` (10s) and `_pollForJobStart` (5s while arrived dialog open) MUST handle: `status == 'completed'` (`popUntil` home), `status == 'pending_completion'` (navigate to `CustomerActiveJobScreen` via passthrough), `status == 'arrived' && started_at != null` (navigate to `CustomerActiveJobScreen` via passthrough), `status == 'arrived' && started_at == null` (show arrived dialog). `_navigateToActiveJob` accepts optional `passthroughData` from the poll cycle — the assignment row's `started_at` is authoritative; `getCustomerActiveJob` is a supplement for joined REQUEST fields (`booked_hours`, `address`) only.
  ...
  - `ActiveJobScreen`: guard active job with countdown timer. Shows remaining hours/minutes from `started_at` + `booked_hours`. Location map placeholder. Complete job button. Chat/Call shortcuts. **Polling cancellation pattern (BUG-016 Task 30):** Both `_resyncTimer` (30s) and `_pollStatus` (3s when `pending_completion`) treat `data == null` from `fetchActiveJobData` as terminal — calls `_handleNullResultAsTerminal()` helper which branches on `_isPendingCompletion`: `true` → success snackbar + pop, `false` → cancellation snackbar + pop. Backend `/guard/active-job` filters out `'completed'`/`'cancelled'` so null = terminal.
...
- ❌ ห้ามให้ ApiClient `publicPaths` ใช้ `.contains()` matching อีก — ต้องเป็น `==` exact equality เท่านั้น มิฉะนั้น sub-paths เช่น `/auth/profile/guard/document/{type}` จะ inherit public-skip rule จาก `/auth/profile/guard` (parent) ทำให้ Bearer header ไม่ถูก attach (latent auth bypass risk)
- ❌ ห้ามแสดง presigned MinIO URL หรือ avatar URL บน mobile โดยไม่ผ่าน `ApiClient.rewriteMediaHost()` — `localhost` ที่ backend ฝังลงไปใช้งานไม่ได้บน Android Emulator + physical device. ต้อง wrap ทุกจุดที่ parse `avatar_url` / doc URLs / chat attachments
- ❌ ห้าม `auth.users.avatar_url` เก็บ presigned URL (URL หมดอายุภายใน 1 ชม.) — ต้องเก็บ **S3 key** (เช่น `profiles/avatars/{user_id}.jpg`) แทน, แล้วให้ `get_profile()` presign on read ผ่าน `finalize_avatar_url()` (presign cheap — local HMAC, ไม่มี network call)
- ❌ ห้าม `update_avatar()` / `update_guard_document()` ข้าม `validate_image_magic_bytes()` หรือ size check — ต้อง validate size ก่อน magic bytes เสมอ (ห้าม allocate memory ก่อน reject)
- ❌ ห้าม `update_guard_document()` (guard self-service) revert `users.approval_status` กลับเป็น pending — guard ที่ approved แล้วเปลี่ยนรูปเอกสารได้อิสระ ไม่ต้องผ่าน admin re-review (product policy 2026-04-30)
- ❌ ห้าม `update_guard_info()` accept user role อื่นนอกจาก `"guard"` — ต้อง `Forbidden` เพื่อกัน customer/admin/null-role ใช้ endpoint นี้แก้ `auth.guard_profiles` แถวอื่นโดยบังเอิญ
- ❌ ห้าม mobile `profile_settings_screen.dart` ใส่ `readOnly: true` กับฟิลด์อื่นนอกจาก phone — guard แก้ได้ทุกฟิลด์ (ชื่อ, อีเมล, ที่อยู่, เพศ, วันเกิด, ประสบการณ์, workplace, bank, emergency contact). Phone เป็น login identifier ห้ามแก้ผ่าน UI นี้ (ต้องผ่าน re-OTP flow ที่ยังไม่ได้ implement)
- ❌ ห้าม mobile mutate `_dobController.text` หรือ `_genderController.text` ใน `build()` (เช่น auto-format ISO → "d MMM yyyy") — เพราะจะ overwrite user input ทุก rebuild. ใช้ `genderDisplay()` / `dobDisplay()` helper return display string แล้ว wrap ใน `AbsorbPointer` + `GestureDetector` → date picker / bottom sheet picker. controller เก็บ raw value (`"male"`, `"1990-05-15"`) ตลอด
...
- ❌ ห้าม `process_refund()` (admin) allow transition ออกจาก `processed` หรือ `skipped` — ต้อง `Conflict` เสมอ; ต้องการ "reopen refund" ต้องแยก flow/endpoint (ไม่อยู่ใน scope migration 042) — ป้องกัน audit trail drift
- ❌ ห้าม `process_refund()` accept `action='process'` โดยไม่มี `reference` หรือ reference เป็น empty/whitespace — ต้อง reject BadRequest; ค่านี้คือ bank slip # ที่เก็บเพื่อ audit
- ❌ ห้าม admin refund endpoints (`GET /admin/wallet/*`, `GET /admin/payments*`, `GET /admin/refunds*`, `PUT /admin/refunds/*`) ไม่ตรวจ `user.role == "admin"` — ต้องใช้ `require_admin()` helper บรรทัดแรกของ handler (403 ถ้าไม่ใช่ admin)
- ❌ ห้าม `prorate_payment_in_tx()` update `refund_amount` โดยไม่ set `refund_status='pending'` พร้อมกัน — ต้อง atomic ใน same UPDATE (`refund_status = CASE WHEN $4 > 0 THEN 'pending' ELSE refund_status END`); แยก 2 queries เสี่ยง crash กลางทางทำให้ refund_amount>0 แต่ไม่โผล่ใน admin queue
...
- ❌ ห้ามกำหนด `auth.role != 'guard'` แล้ว block ทุก path — ต้องให้ fall through ไป registration flow ที่ลึกลงไป (line 156+ ของ `RoleSelectionScreen`) เพื่อให้ user upgrade เป็น guard ได้ผ่าน registration form (BUG-023 Tasks 36+37)
- ❌ ห้าม `CameraFit.bounds(...)` ใน tracking maps โดยไม่กำหนด `maxZoom: 16` — เมื่อ guard กับ customer อยู่ใกล้กัน (distance < 100m) bounds จะ collapse ไป tile-max zoom (building rooftop) → ไม่เห็น road context. ใช้ทุกที่ที่มี `CameraFit.bounds`: `customer_tracking_screen.dart`, `guard_navigation_screen.dart` (BUG-017, BUG-019)
- ❌ ห้าม `_pollStatus` / `_pollForJobStart` ใน customer tracking screen รู้จักเฉพาะ `status == 'arrived'` — ต้องรองรับ `pending_completion` (navigate ไป `CustomerActiveJobScreen`) + `completed` (popUntil home) ด้วย ไม่งั้น customer ค้างที่ tracking screen ถ้า status progression เกิดเร็วกว่า poll interval (BUG-021 Task 34)
- ❌ ห้าม `_pollStatus` / `_resyncTimer` ใน `ActiveJobScreen` early-return ตอน `data == null` โดยไม่ check `_isPendingCompletion` — backend `/guard/active-job` filter completed ออก, null result เป็น terminal signal ที่ต้อง trigger pop screen (BUG-016 Task 30)
- ❌ ห้าม `_navigateToActiveJob` เรียก `getCustomerActiveJob` แบบ load-bearing — service มี silent catch ที่ return null. Use passthrough data จาก poll cycle (assignment row) เป็น authoritative source ของ `started_at` แทน (BUG-021 Task 34)
- ❌ ห้าม dashboard `_switchTab` ที่ใช้ `IndexedStack` ไม่ refresh provider state ตอน switch — `IndexedStack` mounts ทุก child ครั้งเดียวตอน dashboard mount → `initState` ไม่ trigger อีก → cached list stale. Call `fetchJobs()` ตอน switch ไป index 1, `fetchDashboard()` ตอน switch ไป index 0 (BUG-022 Task 35)
- ❌ ห้าม global `receiveTimeout: 15s` ใน `api_client.dart` ครอบ write endpoints ที่ commit state — `assignGuardToRequest` ต้อง per-request override `Options(receiveTimeout: 30s)` เพราะ DB transaction + mobile network ภายใต้ signal ช้าทำให้ Dio timeout ก่อน response (BUG-020 Task 33)
- ❌ ห้าม `ActiveJobResponse` struct ไม่มี `location_lat` / `location_lng` — frontend `guard_job_detail_screen` ใช้ `_job['location_lat']` เพื่อ navigate; ถ้า `_job` มาจาก `/guard/active-job` แล้วไม่มี field นี้ map button silent-fails. ทุก handler ที่ return `ActiveJobResponse` (`start_job`, `get_active_job`, `get_customer_active_job`) ต้อง SELECT `gr.location_lat`, `gr.location_lng` + populate (BUG-015 Issue B Task 29)